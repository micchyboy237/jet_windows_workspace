...
# NEW IMPORTS for audio preprocessing
import torchaudio
import torch
...
def _preprocess_and_save(
    ...
    """
    Convert numpy array → torch tensor → resample to 16 kHz mono → RMS normalize → save.
    """
    import soundfile as sf # local import for compatibility
    ...
    if orig_sr != 16000:
        resampler = Resample(orig_freq=orig_sr, new_freq=16000)
        waveform = resampler(waveform)
    # RMS loudness normalization (≈ -23 LUFS)
    rms = waveform.abs().mean()
    if rms > 0:
        target = 0.1
        waveform = waveform * (target / rms)

    processed_np = waveform.squeeze(0).numpy()

    # save original (unchanged)
    orig_path = audio_dir / f"{stem}.{audio_format}"
    sf.write(orig_path, array, samplerate=orig_sr)

    # save preprocessed version
    preprocessed_path = preprocessed_dir / f"{stem}.{audio_format}"
    sf.write(preprocessed_path, processed_np, samplerate=16000)
...
def extract_parquet_data(
    ...
    audio_preprocessed_dir = output_dir / "audio_preprocessed"
    audio_dir.mkdir(exist_ok=True)
    audio_preprocessed_dir.mkdir(exist_ok=True)

    # NEW — collect text column values
    json_data = {col: [] for col in text_columns}
    json_data["audio"] = []                 # original
    json_data["audio_preprocessed"] = []    # preprocessed
    ...
    with Progress(
        ...
        for idx, row in enumerate(samples.itertuples(index=False), start=1):
            ...
            if not audio_data:
                ...
                json_data["audio_preprocessed"].append(None)
            ...
            try:
                ...
                _preprocess_and_save(
                    ...
                    stem=stem,
                    audio_dir=audio_dir,
                    preprocessed_dir=audio_preprocessed_dir,
                    audio_format=audio_format,
                )
                json_data["audio"].append(f"{stem}.{audio_format}")
                json_data["audio_preprocessed"].append(f"{stem}.{audio_format}")
            except Exception as e:
                log.error(f"Failed to process audio {stem}: {e}")
                json_data["audio"].append(None)
                json_data["audio_preprocessed"].append(None)
                progress.advance(task)
                continue
    ...
    # --- Metadata additions for preprocessed audio ---
    meta.update({
        "audio_original_directory": str(audio_dir.relative_to(output_dir)),
        "audio_preprocessed_directory": str(audio_preprocessed_dir.relative_to(output_dir)),
        "preprocessing": {
    ...
    rprint(f"   Original audio  → [green]audio/{len(list(audio_dir.iterdir()))} files[/green]")
    rprint(f"   Preprocessed    → [bold magenta]audio_preprocessed/{len(list(audio_preprocessed_dir.iterdir()))} files[/bold magenta]")
...
if __name__ == "__main__":
    ...
    DEFAULT_PARQUET = r"C:\Users\druiv\.cache\huggingface\hub\datasets--japanese-asr--whisper_transcriptions.reazon_speech_all\snapshots\96995b6abe6f447be95f4d6b7daa36476b809b46\subset_1.0\train-00000-of-00025.parquet"
    # up to train-00001-of-00025.parquet
