...
# System prompt without examples (examples moved to message history)
...
# Zero-shot examples as separate user/assistant message pairs
TRANSLATION_EXAMPLES = [
    # Basic examples
    ("こんにちは。", "Hello."),
    ("クソッ！また負けた。", "Shit! I lost again."),
    ("今日は暑いね。", "It's hot today."),
    # おまんこ / まんこ
    ...
    ("巨乳を揉みながらおしりの穴を舐めて", "Squeeze my big tits while licking my asshole."),
    ("ザーメンを顔とおっぱいにかけて", "Cum on my face and tits."),
]


def build_translation_messages(japanese_text: str) -> list:
    """Build messages array with system prompt, zero-shot examples, and user text."""
    messages = [{"role": "system", "content": TRANSLATION_SYSTEM_PROMPT}]
    
    # Add each zero-shot example as a separate user/assistant pair
    for ja, en in TRANSLATION_EXAMPLES:
        messages.append({"role": "user", "content": f"Japanese: {ja}\nEnglish:"})
        messages.append({"role": "assistant", "content": en})
    
    # Add the actual user query
    messages.append({"role": "user", "content": f"Japanese: {japanese_text}\nEnglish:"})
...
def example_translation_chat(llm: Llama) -> None:
    ...
    messages = build_translation_messages(japanese_text)
...
def example_multi_explicit_translations(llm: Llama) -> None:
    ...
    for index, (japanese, expected) in enumerate(examples, start=1):
        ...
        messages = build_translation_messages(japanese)
