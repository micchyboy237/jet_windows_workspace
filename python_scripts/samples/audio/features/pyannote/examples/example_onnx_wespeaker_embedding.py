"""
example_onnx_wespeaker_embedding.py
────────────────────────────────────
Speaker embedding extraction using ONNXWeSpeaker (WeSpeaker ResNet34-LM).
...
log = logging.getLogger("wespeaker")
...
def load_audio(audio_path: str | Path, target_sample_rate: int = 16000) -> torch.Tensor:
    ...
    target_sample_rate:
        Target sample rate in Hz (WeSpeaker expects 16 000).
...
def cluster_speakers(
    ...
    try:
        ...
        distance_threshold = 1.0 - threshold  # cosine distance
...
def _build_report_lines(
    ...
    lines.append("  WeSpeaker Embedding – Similarity Report")
...
MODEL_NAME = "hbredin/wespeaker-voxceleb-resnet34-LM"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Speaker embedding extraction and similarity with WeSpeaker ONNX.",
    ...
    console.print(
        Panel.fit(
            f"[bold]WeSpeaker ONNX embedding[/bold]\n"
    ...
    with Progress(
        ...
        for p in resolved:
            w = load_audio(p).squeeze(0)  # (1, T)
            durations.append(audio_duration_seconds(w.unsqueeze(0)))
            waveforms.append(w)
            progress.advance(task)

    # Pad to uniform length and build masks
    ...
    batch_audio = torch.stack(padded)   # (N, 1, T)
    batch_masks = torch.stack(masks)    # (N, T)
...
# ── Fallback path resolver (used when audio_utils is not on the path) ─────────
