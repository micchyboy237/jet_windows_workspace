"""
example_native_pyannote_speaker_embedding.py
─────────────────────────────────────────────
Speaker embedding extraction using PyannoteAudio native embedding model.
...
def load_audio(audio_path: str | Path, target_sample_rate: int = 16000) -> torch.Tensor:
    ...
    target_sample_rate:
        Target sample rate in Hz (pyannote/embedding expects 16 000).
...
def _build_report_lines(
    ...
    lines.append("  PyannoteAudio Embedding – Similarity Report")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Speaker embedding extraction and similarity with PyannoteAudio native embedding.",
    ...
    console.print(
        Panel.fit(
            f"[bold]PyannoteAudio Native Embedding[/bold]\n"
