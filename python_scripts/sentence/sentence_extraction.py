...
if __name__ == "__main__":
    ...
    # ─── Sample texts ────────────────────────────────────────────────────────────
    SINGLE_TEXT = """Machine learning is fun. But sometimes it's hard!
This is the second sentence. And this is the third one...

A new paragraph starts here. It has multiple sentences too. Right?

• This might be a list item
• Another bullet
...
Models like transformers changed everything in 2017.

We are living in exciting times.""",
        "Short. Very short. One more.",
    ]

    # ─── Demonstration cases ─────────────────────────────────────────────────────
    if len(sys.argv) > 1 and sys.argv[1] in ["--help", "-h"]:
        console.print("""
Usage examples (run directly):

    python sentence_extraction.py              → basic single text
    python sentence_extraction.py --paragraph  → paragraph mode
    python sentence_extraction.py --valid      → only valid sentences
    python sentence_extraction.py --multi      → multiple documents
    python sentence_extraction.py --debug      → debug logging
""")
        sys.exit(0)
    ...
    try:
        if mode == "basic":
            print_example_header("Basic usage — single text")
            ...
            print_example_header("Paragraph segmentation mode")
            ...
            print_example_header("Valid sentences only (filtered)")
            sentences = extract_sentences(
                SINGLE_TEXT + "\n\nREFERENCES\nAPPENDIX\n-----\nFooter text",
                model_name="sat-12l-sm",
                valid_only=True,
            ...
            console.print(f"[green]Found {len(sentences)} valid sentences[/green]\n")
            for i, s in enumerate(sentences, 1):
                console.print(f"[dim]{i:2d}.[/dim] {s}")

        elif mode == "multi":
            print_example_header("Multiple documents at once")
            all_sentences = extract_sentences(
                MULTI_TEXTS,
                model_name="sat-12l-sm",
            ...
            console.print(
                f"[green]Extracted {len(all_sentences)} sentences total[/green]\n"
            )
            for i, s in enumerate(all_sentences, 1):
                console.print(f"[dim]{i:2d}.[/dim] {s}")

        elif mode == "debug":
            print_example_header("Debug mode example")
            console.print("[yellow]Running with debug=True → detailed logs[/yellow]\n")
            sentences = extract_sentences(
                SINGLE_TEXT,
                model_name="sat-12l-sm",
                debug=True,  # ← shows model loading, device, etc.
                valid_only=True,
                use_gpu=True,
            )
            console.print("\n[green]Final sentences:[/green]")
            for s in sentences:
                console.print(f"• {s}")
