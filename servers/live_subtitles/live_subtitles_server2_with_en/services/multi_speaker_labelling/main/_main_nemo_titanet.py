...
# Map hex colors to rich color names (fallback to hex strings for custom colors)
RICH_SPEAKER_COLORS = [
    "blue", "dark_orange", "green", "red",
    "magenta", "gold3", "plum3", "grey70",
]

BAR_WIDTH = 25  # Compact bar for console
...
def print_results(results: MultiSpeakerResult, output_dir: Path, assign_threshold: float = 0.55) -> None:
    ...
    tl_table.add_column("", width=8)  # Icons column
    ...
    for idx, seg in enumerate(timeline):
        ...
        # Build file:// URLs for ctrl+click
...
def save_outputs(results: MultiSpeakerResult, audio_path: str, output_dir: Path) -> None:
    ...
    # ── Step 1: Extract per-segment audio snippets via ffmpeg ──
    ...
    # ── Step 2: Global segments.json summary ──
    global_summary = {
    ...
    # ── Step 3: Static timeline plot (PNG) ──
    ...
    # ── Step 4: Interactive HTML timeline ──
