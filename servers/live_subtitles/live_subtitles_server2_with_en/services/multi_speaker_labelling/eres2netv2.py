"""
Automatic multi-speaker labeling using ModelScope/FunASR ERes2NetV2 embeddings
(iic/speech_eres2netv2_sv_zh-cn_16k-common).

Difference from the TitaNet-Large version (temp2.py):
  - Embeddings come from FunASR's `AutoModel` loading the ERes2NetV2 checkpoint,
    instead of NeMo's `EncDecSpeakerLabelModel`. This is the documented way to pull
    raw embeddings from this specific ModelScope model (FunASR's spk_model interface).
  - Embedding dimension is 192 - same family size as TitaNet, so downstream clustering
    code needed no changes.
  - No official cosine-similarity "same speaker" threshold is published for this exact
    checkpoint (unlike NeMo TitaNet's documented 0.70 default), so thresholds here start
    from temp2's empirically-recalibrated real-audio values (0.55) rather than a guessed
    number. Use the inter-cluster similarity log (see merge_similar_clusters) to retune
    against your own recordings.
  - This model is trained on Mandarin (zh-CN) speech. It can still work on other
    languages since speaker embeddings are largely language-agnostic, but accuracy is
    only validated for Chinese in the official docs.

Clustering / merging / timeline logic is unchanged from temp1.py/temp2.py - it only
depends on embedding vectors, not on which model produced them.
"""
...
logger = logging.getLogger("speaker_labeler_eres2netv2")


class SpeakerAutoLabelerERes2NetV2:
    """
    Automatic speaker labeling and centroid extraction using ModelScope/FunASR
    ERes2NetV2 embeddings. Fully automatic with intelligent cluster merging
    (same downstream pipeline as the pyannote and TitaNet versions).
    """
    ...
    def __init__(
        ...
        """
        Initialize speaker labeler with a FunASR ERes2NetV2 model.

        Args:
            model_name: ModelScope model id ("iic/speech_eres2netv2_sv_zh-cn_16k-common").
            duration: Sliding window duration in seconds. Kept the same 1.5s default as
                the TitaNet version since ERes2NetV2 is also a short-duration-optimized
                speaker verification model (its whole design goal, per its paper, is
                staying accurate on short 2-3s clips).
            step: Sliding window step in seconds (50% overlap by default).
            batch_size: How many windows to embed per FunASR generate() call. Kept modest
                (16) to fit comfortably in 6GB VRAM (e.g. GTX 1660).
            min_energy_percentile: Windows whose RMS energy falls below this percentile
                ...
                to disable.
            device: "cuda:0" or "cpu". Auto-detected if not given.
        """
        # Imported here so the rest of this module (clustering/merging logic) can be
        # unit-tested or reused even in environments without funasr installed.
        from funasr import AutoModel

        self.device = device or ("cuda:0" if torch.cuda.is_available() else "cpu")
        logger.info(f"Loading FunASR model: {model_name} (device={self.device})")
        self.model = AutoModel(model=model_name, device=self.device, disable_update=True)

        self.duration = duration
        self.step = step
        self.batch_size = batch_size
        self.min_energy_percentile = min_energy_percentile
        self.sample_rate = 16000  # ERes2NetV2 expects 16kHz mono input
        logger.info(
    ...
    def extract_embeddings(self, audio_path: str):
        """Slice audio into sliding windows and extract an ERes2NetV2 embedding per window."""
        ...
        for batch_start in range(0, len(starts), self.batch_size):
            ...
            # FunASR's AutoModel.generate() takes raw numpy waveforms (or a batch/list of
            # them) plus the sample rate - there's no separate tensor-batching API like
            # NeMo's forward(), so we hand it a list of numpy windows directly.
            batch_signals = [
                waveform[s: s + window_samples].numpy().astype(np.float32)
                for s in batch_starts
            ]

            batch_results = self.model.generate(input=batch_signals, fs=self.sample_rate)
            for res in batch_results:
                # spk_embedding comes back as a torch tensor of shape [1, 192] (still on
                # GPU if device="cuda:0"), not a numpy array - must .cpu() before .numpy()
                emb_tensor = res["spk_embedding"]
                if isinstance(emb_tensor, torch.Tensor):
                    emb = emb_tensor.detach().cpu().numpy().reshape(-1)
                else:
                    emb = np.asarray(emb_tensor).reshape(-1)
    ...
    def log_similarity_diagnostics(self, embeddings):
        """
        Log the full pairwise cosine-similarity distribution across all extracted windows,
        BEFORE any clustering happens. This is independent of how many clusters get picked
        (unlike the inter-cluster log in merge_similar_clusters, which only reflects
        whatever the initial clustering guessed), so it's a much more honest picture of
        how well this model+audio combination actually separates speakers.

        Read it like this:
          - If most pairs cluster tightly around one high value (e.g. everything in the
            0.5-0.6 range with a narrow spread), the model isn't discriminating speakers
            well for this audio - no threshold will fix that, you need a different model
            (this is what happened with a Mandarin-trained model on non-Mandarin audio).
          - If there's a visible gap between a lower hump (different speakers) and a
            higher hump (same speaker), pick merge/assign thresholds that sit in that gap.
        """
        n = len(embeddings)
        sims = embeddings @ embeddings.T
        off_diag = sims[np.triu_indices(n, k=1)]
        percentiles = np.percentile(off_diag, [5, 25, 50, 75, 95])
        logger.info(
            "Raw pairwise similarity across ALL windows (pre-clustering) - "
            f"p5={percentiles[0]:.3f}, p25={percentiles[1]:.3f}, p50={percentiles[2]:.3f}, "
            f"p75={percentiles[3]:.3f}, p95={percentiles[4]:.3f}, spread={percentiles[4] - percentiles[0]:.3f}"
        )
        if percentiles[4] - percentiles[0] < 0.15:
            logger.warning(
                "  Very narrow similarity spread (<0.15) - this model may not be "
                "discriminating speakers well for this audio. No threshold choice can "
                "fully fix that; consider a different embedding model for this content."
            )
        return percentiles
    ...
    def merge_similar_clusters(self, embeddings, labels, similarity_threshold=0.55):
        ...
        NOTE: No official cosine-similarity threshold is published for this ERes2NetV2
        checkpoint. 0.55 is carried over from the value we empirically recalibrated for
        TitaNet on real conversational audio (temp2.py) - a reasonable starting point
        since both are 192-dim short-window embeddings on the same kind of problem, but
        NOT a verified number for this specific model. Check the inter-cluster similarity
        log line printed below every run and adjust --merge-threshold to sit clearly
        above your data's between-speaker scores and below its same-speaker scores.
        """
    ...
    def assign_speaker_labels(self, embeddings, centroids, threshold=0.55):
        ...
        NOTE: same caveat as merge_similar_clusters - no official threshold is published
        for this checkpoint. 0.55 is the TitaNet real-audio starting point, not a
        verified ERes2NetV2 number. Check the "Frame assignment" percentages printed at
        the end of a run: if too many windows are landing below threshold, lower this;
        if speakers are bleeding into each other, raise it.
        """
        speaker_ids = []
        confidences = []
...
def main(
    ...
    """Main execution flow - FULLY AUTOMATIC WITH SMART MERGING (ERes2NetV2 version)."""
    logger.info("=" * 60)
    logger.info("AUTO SPEAKER LABELING WITH ERES2NETV2 + INTELLIGENT MERGING")
    logger.info("=" * 60)
    logger.info(f"Model: {model_name}")
    logger.info(f"Merge threshold: {merge_threshold} (unverified for this model - see docstring)")
    logger.info(f"Assignment threshold: {assign_threshold} (unverified for this model - see docstring)")
    logger.info(f"Silence filter percentile: {min_energy_percentile}")

    labeler = SpeakerAutoLabelerERes2NetV2(
    ...
    labeler.log_similarity_diagnostics(embeddings)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Automatic speaker labeling with ModelScope/FunASR ERes2NetV2 embeddings",
    ...
    parser.add_argument(
        "--model-name", type=str, default="iic/speech_eres2netv2_sv_zh-cn_16k-common",
        help="ModelScope/FunASR pretrained speaker embedding model id"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Number of windows sent per FunASR generate() call (lower this if you hit GPU out-of-memory)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Similarity threshold for merging speaker clusters. Starting point carried over from TitaNet real-audio testing - no official value is published for ERes2NetV2, so check the inter-cluster similarity log and retune for your data"
    )
    parser.add_argument(
        "-a", "--assign-threshold", type=float, default=0.55,
        help="Minimum similarity threshold for assigning a frame to a speaker. Same caveat as --merge-threshold: unverified starting point, retune using the printed frame-assignment percentages"
    )
