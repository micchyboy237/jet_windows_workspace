...
# Speech wave preprocessing for higher speech audio
...
class SegmentSpeakerLabeler:
    ...
    def __init__(
        ...
        # Speech wave filtering configuration
    ...
    def create_new_speaker(
        ...
        IMPORTANT: This method adds the initial embedding and sets
        segment_count = 1. Callers should NOT call update_reference()
        immediately after this method for the same embedding.
        """
    ...
    def update_reference(
        ...
        IMPORTANT: Only call this for EXISTING speakers that have been
        matched to a new segment. Do NOT call this for newly created
        speakers (create_new_speaker already adds the embedding).
        """
        ...
        # Trim old embeddings if exceeding max
    ...
    def _get_speaker_categories(self) -> Dict[str, Dict]:
        ...
        for label, ref in self._speakers.items():
            ...
            if ref.segment_count >= self.mature_segment_count:
                ...
                # 3-4 segments = active_young (between young and mature)
                active_young[label] = ref
    ...
    def _cleanup_orphan_speakers(self, current_timestamp: float) -> int:
        ...
        Only targets speakers with segment_count <= young_segment_count
        that have been inactive for > 2x temporal_smoothing_window.
        """
        removed = 0
        ...
        for label in labels_to_check:
            ...
            # Only clean up young speakers that have been inactive for a while
            if (
                ...
                if ref.has_valid_centroid and len(self._speakers) > 1:
                    # Try to merge with a mature speaker first
                    ...
                    if best_match and best_match != label and best_score > 0.50:
                        ...
                        # Only remove single-segment orphans after long inactivity
                        if self.debug:
    ...
    def label_segments(
        ...
        If self.use_speech_wave_filtering is True, audio is automatically
        filtered to high-confidence speech portions before computing embeddings.
        No external speech_waves parameter needed.
        """
        self.total_segments_processed += 1
        if top_k is None:
            top_k = self.top_k_speakers

        # Auto-filter if enabled — no external data needed
        ...
        if should_create or not top_matches:
            # FIX: create_new_speaker() already adds embedding and sets
            # segment_count = 1. Do NOT call update_reference() again.
            ...
            # Add alternative matches for context
            for match in top_matches:
            ...
            # FIX: Only update_reference for EXISTING speaker matches
            ...
            for i, match in enumerate(top_matches):
                ...
                # Temporal smoothing for possible matches
                ...
                # Context-based matching (only if confidence is sufficient)
            ...
            # Deduplicate and update reference for the primary match
            results = self._deduplicate_results(results)
            primary_result = results[0]
            
            # FIX: Only call update_reference for existing speakers
    ...
    def merge_speakers(
        ...
        if self.debug:
            console.print(
                f"[yellow]Merged: {label1} + {label2} → {primary_label}[/]"
            )
