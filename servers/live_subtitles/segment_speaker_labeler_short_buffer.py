...
from short_embedding_buffer import (
    ShortEmbeddingBuffer,
    DEFAULT_MIN_DURATION_SECONDS,
    DEFAULT_SIMILARITY_THRESHOLD as DEFAULT_BUFFER_SIMILARITY_THRESHOLD,
)
...
# Short embedding buffer constants
DEFAULT_MIN_EMBEDDING_DURATION: float = DEFAULT_MIN_DURATION_SECONDS
DEFAULT_BUFFER_SIMILARITY: float = DEFAULT_BUFFER_SIMILARITY_THRESHOLD
DEFAULT_MAX_BUFFER_SLOTS: int = 10
DEFAULT_BUFFER_TTL: float = 15.0
DEFAULT_ENABLE_SHORT_BUFFER: bool = True
...
class SegmentSpeakerLabeler:
    ...
    def __init__(
        ...
        enable_short_buffer: bool = DEFAULT_ENABLE_SHORT_BUFFER,
        min_embedding_duration: float = DEFAULT_MIN_EMBEDDING_DURATION,
        buffer_similarity_threshold: float = DEFAULT_BUFFER_SIMILARITY,
        max_buffer_slots: int = DEFAULT_MAX_BUFFER_SLOTS,
        buffer_ttl: float = DEFAULT_BUFFER_TTL,
        debug: bool = False,
    ):
        ...
        # Short embedding buffer configuration
        self.enable_short_buffer = enable_short_buffer
        self.min_embedding_duration = min_embedding_duration
        self.buffer_similarity_threshold = buffer_similarity_threshold
        ...
        # Initialize short embedding buffer
        if self.enable_short_buffer:
            self._short_buffer = ShortEmbeddingBuffer(
                min_duration=self.min_embedding_duration,
                similarity_threshold=self.buffer_similarity_threshold,
                max_slots=max_buffer_slots,
                slot_ttl=buffer_ttl,
                debug=debug,
            )
        else:
        ...
        # Track buffer-promoted segments for statistics
        self.total_buffer_promoted = 0
        self.total_short_buffered = 0
    ...
    def label_segments(
        ...
        Now supports short embedding buffering: segments shorter than 
        min_embedding_duration will be buffered and merged before comparison.
        ...
        Args:
            ...
            top_k: Number of top speakers to return
            segment_duration: Duration of the segment in seconds.
                If None, will be estimated from waveform length.
            
        Returns:
            List of speaker label dictionaries sorted by confidence
        """
        ...
        if segment_duration is None:
            ...
            segment_duration = num_samples / sample_rate
        
        # Compute embedding
        embedding = self.compute_embedding(waveform, sample_rate)
        
        # Check if this is a short segment that should be buffered
        if (
            self.enable_short_buffer
            and self._short_buffer is not None
            and segment_duration < self.min_embedding_duration
        ):
            if self.debug:
                console.print(
                    f"[dim]📦 Short segment ({segment_duration:.2f}s < "
                    f"{self.min_embedding_duration}s) → buffering[/dim]"
                )
            
            self.total_short_buffered += 1
            
            # Add to buffer and check if a slot is ready for promotion
            result = self._short_buffer.add_embedding(
                embedding=embedding,
            ...
            if result is not None:
                # A buffer slot was promoted! Process it as a full embedding
                promoted_embedding, status = result
                self.total_buffer_promoted += 1
                
                if self.debug:
                    console.print(
                        f"[green]🚀 Buffer promoted: "
                        f"processing merged embedding with main system[/green]"
                    )
                
                # Process the promoted embedding through normal flow
                return self._process_full_embedding(
                    embedding=promoted_embedding,
                    timestamp=timestamp,
                    context=context,
                    top_k=top_k,
                )
            else:
                # Segment was buffered, not ready for speaker assignment yet
                # Return a placeholder result indicating buffered status
                return [{
                    "label": "BUFFERED",
                    "confidence": 0.0,
                    "match_type": "buffered_short",
                    "is_primary": True,
                    "is_buffered": True,
            ...
            # Normal or promoted embedding processing
            return self._process_full_embedding(
                embedding=embedding,
                timestamp=timestamp,
                context=context,
    ...
    def _process_full_embedding(
        ...
        """Process a full-quality embedding through the normal speaker labeling flow.
        
        This is the same logic that was previously inline in label_segments,
        extracted to handle both normal and buffer-promoted embeddings.
        
        Args:
            embedding: Speaker embedding (full quality or buffer-promoted)
            timestamp: Segment timestamp
            context: Optional context dictionary
            top_k: Number of top speakers to return
            
        Returns:
            List of speaker label dictionaries
        """
        ...
        if self.debug:
            console.print(
                ...
                f"got {len(top_matches)} top matches[/dim]"
            )
        ...
        if self.debug:
            console.print(
                ...
                f"should_create_new_speaker: {should_create}[/dim]"
            )
        ...
        if should_create or not top_matches:
            ...
            for i, match in enumerate(top_matches):
                ...
                # Temporal smoothing for primary match
                ...
                # Context-based matching for primary
                if is_primary and context and "previous_speaker" in context:
                    prev_speaker = context["previous_speaker"]
                    if (
                        prev_speaker
                        and prev_speaker in all_scores
    ...
    def get_buffer_status(self) -> Optional[Dict]:
        """Get current status of the short embedding buffer.
        
        Returns:
            Buffer statistics dict or None if buffer is disabled
        """
        if self._short_buffer is None:
            return None
        
        return {
            "enabled": self.enable_short_buffer,
            "min_duration": self.min_embedding_duration,
            "statistics": self._short_buffer.get_statistics(),
            "active_slots": self._short_buffer.active_slots,
        }

    def check_and_promote_buffered(self) -> List[Dict]:
        """Check buffer for any ready slots and promote them.
        
        This can be called periodically or when silence is detected
        to ensure buffered embeddings don't wait too long.
        
        Returns:
            List of results from processing promoted embeddings
        """
        if self._short_buffer is None:
            return []
        
        promoted = self._short_buffer.check_for_ready_slots()
        results = []
        
        for embedding, status in promoted:
            self.total_buffer_promoted += 1
            
            # Process through normal flow with current timestamp
            result = self._process_full_embedding(
                embedding=embedding,
    ...
    def _get_latest_timestamp(self) -> float:
        """Get the most recent timestamp from all speakers."""
        return max(
    ...
    def to_dict(self) -> Dict:
        """Serialize the labeler state including buffer state."""
        speakers_data = {}
        for label, ref in self._speakers.items():
            speakers_data[label] = {
                "label": ref.label,
        ...
        result = {
            ...
            "enable_short_buffer": self.enable_short_buffer,
            "min_embedding_duration": self.min_embedding_duration,
            "total_buffer_promoted": self.total_buffer_promoted,
            "total_short_buffered": self.total_short_buffered,
        }
        
        # Include buffer state if enabled
        if self._short_buffer is not None:
            result["short_buffer"] = self._short_buffer.to_dict()
    ...
    def from_dict(cls, data: Dict, embedding_model) -> "SegmentSpeakerLabeler":
        ...
        labeler = cls(
            ...
            enable_short_buffer=data.get("enable_short_buffer", DEFAULT_ENABLE_SHORT_BUFFER),
            min_embedding_duration=data.get("min_embedding_duration", DEFAULT_MIN_EMBEDDING_DURATION),
        )
        ...
        labeler.total_buffer_promoted = data.get("total_buffer_promoted", 0)
        labeler.total_short_buffered = data.get("total_short_buffered", 0)
        ...
        # Restore buffer state if present
        if labeler._short_buffer is not None and "short_buffer" in data:
            labeler._short_buffer = ShortEmbeddingBuffer.from_dict(
                data["short_buffer"],
                min_duration=labeler.min_embedding_duration,
                similarity_threshold=labeler.buffer_similarity_threshold,
                debug=labeler.debug,
            )
